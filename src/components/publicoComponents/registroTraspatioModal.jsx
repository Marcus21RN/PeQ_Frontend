...
import { uploadMediaFile, registrarTraspatio } from '../../services/apiPublico/registroPublico.js';
...
const documentLabels = {
  ...
  foto_rancho: 'Fotografías del Rancho',
};
...
export default function RegistroTraspatioModal({ isOpen, onClose }) {
  ...
  const steps = useMemo(
    () => [
      ...
      { title: 'Documentación Requerida', subtitle: 'Adjunta los documentos requeridos' },
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        nombre_granja: formData.nombre_granja,
      ...
      await registrarTraspatio(payload);
      setToast({ type: 'success', message: 'Registro exitoso. El productor de traspatio fue registrado correctamente.' });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#2d1d12]/50 px-4 py-6 backdrop-blur-[1px]">
      <div className="relative flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-[18px] border border-[#d9d2cc] bg-[#f5f2ee] shadow-2xl">
        <div className="flex items-center justify-between bg-[#2c1405] px-6 py-4 text-white">
          <div>
            <h2 className="font-serif text-2xl font-bold tracking-wide">Rancho de Traspatio</h2>
          ...
          {currentStep === 0 && (
            <div className="space-y-5">
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <Field label="Nombre del Rancho" icon={<House size={15} />} required>
                  <input
                    value={formData.nombre_granja}
                    onChange={(event) => updateField('nombre_granja', event.target.value)}
                    className="w-full rounded-xl border border-[#d7d0c9] bg-[#f8f6f4] px-4 py-3 text-base text-[#2b1c12] outline-none transition focus:border-[#6b8c3a]"
                    placeholder="Rancho El Ejemplo"
                  />
                  {errors.nombre_granja && <ErrorText>{errors.nombre_granja}</ErrorText>}
                </Field>
            ...
            <div className="space-y-5">
              <div className="rounded-2xl border border-[#d7d0c9] bg-[#ffffff] p-5 shadow-sm">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <InfoRow label="Rancho" value={formData.nombre_granja} />
