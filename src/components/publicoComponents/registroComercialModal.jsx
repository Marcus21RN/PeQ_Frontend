...
import { uploadMediaFile, registrarComercial } from '../../services/apiPublico/registroPublico.js';
...
const documentLabels = {
  ...
  acta_constitutiva: 'Acta Constitutiva o Registro Comercial',
  rfc: 'RFC con Constancia de Situación Fiscal',
  comprobante_domicilio: 'Comprobante de Domicilio',
  permisos_sanitarios: 'Permisos Sanitarios (SAGARPA/SENASICA)',
  licencia_uso_suelo: 'Licencia de Uso de Suelo',
  fotos_instalaciones: 'Fotografías de las Instalaciones',
  plan_manejo_sanitario: 'Plan de Manejo Sanitario',
};
...
const initialDocuments = {
  ...
  licencia_uso_suelo: null,
  fotos_instalaciones: null,
  plan_manejo_sanitario: null,
};
...
export default function RegistroComercialModal({ isOpen, onClose }) {
  ...
  const steps = useMemo(
    () => [
      ...
      { title: 'Documentación Requerida', subtitle: 'Adjunta los documentos necesarios para tu registro' },
  ...
  const validateStepTwo = () => {
    const missing = Object.keys(documentLabels).filter((key) => !uploadedFiles[key]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        nombre_rancho: formData.nombre_rancho,
      ...
      await registrarComercial(payload);
      setToast({ type: 'success', message: 'Registro exitoso. El rancho comercial fue registrado correctamente.' });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#2d1d12]/50 px-4 py-6 backdrop-blur-[1px]">
      <div className="relative flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-[18px] border border-[#d9d2cc] bg-[#f5f2ee] shadow-2xl">
        <div className="flex items-center justify-between bg-[#2c1405] px-6 py-4 text-white">
          <div>
            <h2 className="font-serif text-2xl font-bold tracking-wide">Rancho Comercial</h2>
            ...
            <div className="space-y-5">
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <div className="md:col-span-2">
                  <Field label="Dirección Completa" icon={<MapPin size={15} />} required>
                    <input
                ...
                <Field label="Nombre del Rancho" icon={<Building2 size={15} />} required>
                  <input
                    value={formData.nombre_rancho}
                    onChange={(event) => updateField('nombre_rancho', event.target.value)}
                    className="w-full rounded-xl border border-[#d7d0c9] bg-[#f8f6f4] px-4 py-3 text-base text-[#2b1c12] outline-none transition focus:border-[#6b8c3a]"
                    placeholder="Rancho El Ejemplo"
                  />
                  {errors.nombre_rancho && <ErrorText>{errors.nombre_rancho}</ErrorText>}
                </Field>

                <Field label="Capacidad (Animales)" icon={<BadgeCheck size={15} />} required>
                  <input
                    type="number"
                    min="0"
                    value={formData.capacidad_animales}
                    onChange={(event) => updateField('capacidad_animales', event.target.value)}
                    className="w-full rounded-xl border border-[#d7d0c9] bg-[#f8f6f4] px-4 py-3 text-base text-[#2b1c12] outline-none transition focus:border-[#6b8c3a]"
                    placeholder="100"
                  />
                  {errors.capacidad_animales && <ErrorText>{errors.capacidad_animales}</ErrorText>}
                </Field>

                <div className="md:col-span-2">
                  <Field label="Superficie (Hectáreas)" icon={<MapPin size={15} />} required>
                    <input
                      type="number"
                      min="0"
                      step="0.1"
                      value={formData.superficie_hectareas}
                      onChange={(event) => updateField('superficie_hectareas', event.target.value)}
                      className="w-full rounded-xl border border-[#d7d0c9] bg-[#f8f6f4] px-4 py-3 text-base text-[#2b1c12] outline-none transition focus:border-[#6b8c3a]"
                      placeholder="50"
                    />
                    {errors.superficie_hectareas && <ErrorText>{errors.superficie_hectareas}</ErrorText>}
                  </Field>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-5">
              <div className="rounded-2xl border border-[#d7d0c9] bg-[#ffffff] p-5 shadow-sm">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <InfoRow label="Rancho" value={formData.nombre_rancho} />
                  <InfoRow label="Dirección" value={formData.direccion} />
                  <InfoRow label="Capacidad" value={formData.capacidad_animales} />
                  <InfoRow label="Superficie" value={`${formData.superficie_hectareas} ha`} />
                </div>
              </div>
...
function Field({ label, required, icon, children }) {
  return (
    <label className="block text-[#2c1b12]">
      <span className="mb-2 flex items-center gap-2 text-[17px] font-semibold">
        <span className="text-[#5d7e39]">{icon}</span>
        {label}
        {required && <span className="text-[#9d513d]">*</span>}
      </span>
      {children}
    </label>
  );
...
function InfoRow({ label, value }) {
  return (
    <div className="rounded-xl border border-[#e5dfd8] bg-[#f8f6f4] p-3">
      <div className="text-xs font-semibold uppercase tracking-wide text-[#7d665d]">{label}</div>
      <div className="mt-1 text-base font-medium text-[#2b1c12]">{value || '—'}</div>
    </div>
  );
}

function ErrorText({ children }) {
  return <span className="mt-2 block text-sm font-medium text-[#9d513d]">{children}</span>;
}
