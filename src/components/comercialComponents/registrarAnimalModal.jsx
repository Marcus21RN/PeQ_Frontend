// src/components/comercial/ModalRegistrarAnimal.jsx
...
export default function ModalRegistrarAnimal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      }, 300); // Pequeño delay para que no se vea el cambio de vista mientras desaparece
    }
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      setIsSuccess(true); // Cambiamos a la vista de éxito dentro del mismo modal
    }, 800);
  };
  ...
  return (
    ...
    >
      <div 
        className="bg-[#FDFDFB] rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden font-serif"
      ...
      >
        ...
        {!isSuccess && (
          <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between shrink-0 bg-white">
            <div>
              <h2 className="text-2xl font-bold text-[#3B2211]">Registrar Nuevo Animal</h2>
              <p className="text-gray-500 mt-1 font-sans text-sm">Completa la información del animal para su registro en el sistema</p>
            </div>
            <button 
              onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 p-8">
          ...
          {isSuccess ? (
            ...
            /* ========================================== */
            <div className="flex flex-col items-center justify-center h-full text-center max-w-lg mx-auto py-12 animate-in zoom-in-95 duration-300">
              <div className="w-24 h-24 bg-[#F0F4EC] rounded-full flex items-center justify-center mb-6">
              ...
              <div className="flex flex-col w-full gap-4 font-sans">
                <button 
                  ...
                  className="w-full py-3.5 border-2 border-[#5C743D] text-[#5C743D] rounded-xl font-bold hover:bg-[#F0F4EC] transition-colors"
                >
                  Registrar Otro Animal
                </button>
                <button 
                  onClick={onClose}
                  className="w-full py-3.5 bg-[#5C743D] text-white rounded-xl font-bold hover:bg-[#4A5D31] transition-colors shadow-md"
                >
            ...
            /* ========================================== */
            <form id="form-registro-animal" onSubmit={handleSubmit} className="space-y-10 font-sans">
              
              {/* SECCIÓN 1: Información Básica */}
              <section>
                <h3 className="text-lg font-bold text-[#3B2211] font-serif mb-5 border-b border-gray-100 pb-2">Información Básica</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <select name="raza" value={formData.raza} onChange={handleInputChange} required className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#5C743D] outline-none text-gray-700 bg-white">
                  ...
                  <div>
                    ...
                    <input type="number" step="0.1" name="edad" value={formData.edad} onChange={handleInputChange} required placeholder="Ej: 2.5" className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#5C743D] outline-none text-gray-700" />
                  </div>
                </div>
              </section>
              ...
              <section>
                <h3 className="text-lg font-bold text-[#3B2211] font-serif mb-5 border-b border-gray-100 pb-2">Datos Físicos</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <LabelRequerido texto="Peso Estimado (kg)" />
                    <input type="number" name="peso" value={formData.peso} onChange={handleInputChange} required placeholder="Ej: 450" className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#5C743D] outline-none text-gray-700" />
                  ...
                  <div>
                    ...
                    <input type="text" name="color" value={formData.color} onChange={handleInputChange} required placeholder="Ej: Negro con blanco" className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#5C743D] outline-none text-gray-700" />
                  </div>
                  <div>
              ...
              <section>
                <h3 className="text-lg font-bold text-[#3B2211] font-serif mb-5 border-b border-gray-100 pb-2">Tipo de Producción</h3>
              ...
              {/* SECCIÓN 4: Documentación y Fotos */}
              <section>
                <h3 className="text-lg font-bold text-[#3B2211] font-serif mb-5 border-b border-gray-100 pb-2">Documentación y Fotografías</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div className="border-2 border-dashed border-gray-300 rounded-2xl p-6 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-gray-50 transition-colors">
                    <Camera className="w-8 h-8 text-gray-400 mb-2" />
                    <p className="text-sm font-bold text-gray-700 mb-1">Fotografía Frontal *</p>
                    <p className="text-xs text-gray-400 mb-2">Vista completa del animal</p>
                    <span className="text-xs text-[#A4B15C] font-semibold">Seleccionar imagen</span>
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-2xl p-6 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-gray-50 transition-colors">
                    ...
                    <p className="text-xs text-gray-400 mb-2">Vista de perfil</p>
                    <span className="text-xs text-[#A4B15C] font-semibold">Seleccionar imagen</span>
                    <input type="file" className="hidden" accept="image/*" />
                  </div>
                </div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-2xl p-6 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-gray-50 transition-colors">
                  ...
                  <p className="text-xs text-gray-400 mb-2">Certificados de vacunación, pedigree, etc. (Opcional)</p>
                  <span className="text-xs text-[#A4B15C] font-semibold">Seleccionar archivos</span>
              ...
              <section>
                <h3 className="text-lg font-bold text-[#3B2211] font-serif mb-5 border-b border-gray-100 pb-2">Observaciones Adicionales</h3>
                <textarea 
                  name="observaciones" 
        ...
        {/* Footer del Modal (Botones) - Solo visible si no es éxito */}
        {!isSuccess && (
          <div className="bg-white px-8 py-5 border-t border-gray-100 flex flex-col sm:flex-row gap-4 shrink-0 font-sans">
            <button 
              type="button"
              onClick={onClose}
              className="flex-1 py-3.5 border border-gray-300 rounded-xl text-center font-bold text-gray-700 hover:bg-gray-50 transition-colors"
            >
              Cancelar
            </button>
            <button 
              form="form-registro-animal"
              type="submit"
              disabled={isLoading} 
              className="flex-1 py-3.5 bg-[#5C743D] text-white rounded-xl font-bold hover:bg-[#4A5D31] transition-colors disabled:bg-[#5C743D]/70 shadow-sm"
            >
