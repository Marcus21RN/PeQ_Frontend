...
import { getDashboardComercial } from '../../services/apiComercial/dashboarProductor.js';
...
export default function PanelPrincipalComercial() {
  ...
  useEffect(() => {
    ...
    getDashboardComercial()
      ...
      .catch((err) => {
        console.error('Error cargando datos del panel comercial:', err);
        if (isMounted) {
        ...
        {desglose_categorias && desglose_categorias.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 font-sans">
            {desglose_categorias.map((cat, index) => (
              <Link 
                key={index} 
                to={`/comercial/animales?tipo=${encodeURIComponent(cat.categoria)}`} 
